...
SOCKET connect_to_server() {
  ...
  server_addr.sin_addr.s_addr =
      inet_addr(SERVER_IP); // 문자열 IP를 네트워크 주소로 변환
...
static int receive_and_parse_http(SOCKET s, char *out_body, int out_body_size) {
  ...
  // 1. 헤더 경계(\r\n\r\n)까지 반복 수신
  while ((r = recv(s, response + total_received, BUF_SIZE - 1 - total_received,
                   0)) > 0) {
    total_received += r;
...
// HTTP 메시지 작성
void handle_http_request(SOCKET s, const char *method, const char *path,
...
int main(void) {
  ...
  if (connected_socket != INVALID_SOCKET) {
    ...
    _snprintf_s(list_header, sizeof(list_header), _TRUNCATE, "X-User: %s",
                user);
  ...
  if (connected_socket != INVALID_SOCKET) {
    ...
    _snprintf_s(detail_header, sizeof(detail_header), _TRUNCATE,
                "X-Mail-Id: %s", mail_id);
